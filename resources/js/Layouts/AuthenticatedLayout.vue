...
const canViewTaskTypes = computed(() => hasPermission('task-types.read'));
...
const canViewAdmin = computed(() => canViewUsers.value || canViewRoles.value || canViewTicketTypes.value || canViewTaskTypes.value);
const isOnAdminRoute = computed(() => route().current('admin.users.*') || route().current('admin.roles.*') || route().current('admin.ticket-types.*') || route().current('admin.task-types.*'));
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                ...
                <div v-if="canViewAdmin" class="mt-2">
                    ...
                    <div v-show="isAdminOpen" class="ml-4 mt-1 space-y-1">
                        ...
                        <Link v-if="canViewTaskTypes" :href="route('admin.task-types.index')"
                            :class="[route().current('admin.task-types.*') ? 'bg-emerald-600 text-white dark:text-zinc-950 font-semibold' : 'text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-900 dark:hover:text-zinc-100', 'flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200']">
                            <svg class="mr-3 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
